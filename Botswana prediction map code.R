...
# read in data
df <- read.csv("/Users/amandairish/Desktop/Malaria project/Sentinel data known and pred.csv")
df_lg <- read.csv("/Users/amandairish/Desktop/Malaria project/Sentinel data with roof type pred.csv")
df_v <- read.csv("/Users/amandairish/Desktop/Malaria project/Validation sentinel data with roof type pred.csv")
...
B_Adm_1 <- readOGR("/Users/amandairish/Downloads/BWA_adm_shp")

B_Adm_1 <- raster::getData("GADM", country="BWA", level=1)
...
colors <- c("#76FF03", "#FFFF00", "#D500F9") #bright green, bright purple, bright yellow (not official color names)
...
# plot large dataset over a satellite image - Gaborone

lg_map_sat <- leaflet(lg_SPDF) %>%
  setView(25.93452602, -24.61357581, zoom = 17) %>%  #Gaborone
...
mapshot(lg_map_sat, file = "full_data_sat_map_zoom_2.png")
