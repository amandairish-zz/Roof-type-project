...
trainIndex <- createDataPartition(swazi.testtrain$LULC, p = 0.8,
                                  list = FALSE,
                                  times = 1)
...
# xgboost
tune.xgboost <- list(ntrees = c(100, 200, 500),
                    max_depth = 1:4,
...
# random forest
tune.rf <- list(ntrees = c(100, 200, 500),
                max_depth = 1:6,
...
# fit CV.SL using method.NNloglik since binomial outcome & have had problems
# with convergence using method.AUC
# While this is running check CPU using in Activity Monitor / Task Manager.
# These took about 16 min or so 4/28/19
system.time({
...
fit_T_SL  <- SuperLearner(Y = Y_T,  X = X, SL.library = SL.library, verbose = FALSE, family = binomial(), method = "method.NNloglik")
fit_M_SL  <- SuperLearner(Y = Y_M,  X = X, SL.library = SL.library, verbose = FALSE, family = binomial(), method = "method.NNloglik")
fit_Th_SL <- SuperLearner(Y = Y_Th, X = X, SL.library = SL.library, verbose = FALSE, family = binomial(), method = "method.NNloglik")
...
savehistory(file="SWZ roof type SL with tuning final") # default is ".Rhistory" 

# save the workspace to the file .RData in the cwd 
save.image(file = "SWZ roof type SL with tuning final")
