...
## last updated 5/17/18 by Amanda Irish
...
# load training data
df <- read.csv("SWZ_OSM_Sentinel_112017.csv")
head(df)
...
#df$pred_roof_type <- as.factor(as.character(df$pred_roof_type))
df$LULC <- as.factor(as.character(df$LULC))
levels(df$LULC)

# create validation set from df: 10% stratified random sample of observations
set.seed(123)
...
# SL library
SL.library <- c("SL.randomForest", "SL.rf.moretrees", learners$names, "SL.gbm", 
                "SL.glmnet", "SL.glm", "SL.knn", "SL.gam", "SL.xgboost", 
                learners_xgboost$names, "SL.bayesglm", "SL.mean")
...
# Use recombineCVSL to look at other methods - NNLS & AUC
set.seed(123)

fit_T2  <- CV.SuperLearner(Y = Y_T,  X = X, V = 10, SL.library = SL.library, verbose = FALSE, method = "method.NNLS", family = binomial(), cvControl = list(stratifyCV = TRUE))
fit_T3  <- CV.SuperLearner(Y = Y_T,  X = X, V = 10, SL.library = SL.library, verbose = FALSE, method = "method.AUC", family = binomial(), cvControl = list(stratifyCV = TRUE))

# These don't give summary(fit_T2/T3) - gives the results for NNLS...????
#fit_T2 <- recombineCVSL(fit_T, method = "method.NNloglik", verbose = TRUE, saveAll = TRUE, parallel = "multicore")
...
SL.library2 <- c("SL.gbm", "SL.gam", "SL.xgboost", "SL.xgboost_7", 
                 "SL.glmnet", "SL.xgboost_10", "SL.randomForest_3", 
                 "SL.randomForest")

# Fit SL functions w/ method.auc & new SL library
fit_T_SL  <- SuperLearner(Y = Y_T,  X = X, SL.library = SL.library2, verbose = FALSE, method = "method.AUC", family = binomial())
fit_M_SL  <- SuperLearner(Y = Y_M,  X = X, SL.library = SL.library2, verbose = FALSE, method = "method.AUC", family = binomial())
fit_Th_SL <- SuperLearner(Y = Y_Th, X = X, SL.library = SL.library2, verbose = FALSE, method = "method.AUC", family = binomial())
...
pred_M  <- predict(fit_M_SL, newdata = new_df)
pred_Th <- predict(fit_Th_SL, newdata = new_df) 
...
Classify_vdf <- apply(SL_pred_vdf, 1, function(xx) c("Tile", "Metal", "Thatch")[unname(which.max(xx))])
...
# Write out new dataset with roof predictions
write.csv(df_new, file = "Swazi Sentinel data with roof type pred.csv")

# Save all objects in global environment
save(list = ls(), file = "Swazi SL roof pred global env.RData")

# Save workspace image
save.image(file = "Swazi SL roof pred image.RData")
